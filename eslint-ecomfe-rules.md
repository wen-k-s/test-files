...
-   禁止将常量作为**分支条件判断中的测试表达式**，但允许作为**循环条件判断中的测试表达式**。
...
- 禁止使用**特殊空白符**（比如全角空格），除非是出现在字符串、正则表达式或模版字符串中。
...
- 禁止**直接**使用 hasOwnProperty, isPrototypeOf 或 propertyIsEnumerable

  eslint: [no-prototype-builtins](http://eslint.cn/docs/rules/no-prototype-builtins)

  > 该规则原意是禁止**在直接对象实例上**调用 `Object.prototype` 的一些方法。咱们eslintrc.js文件里的配置及相关注释应该是理解错误，应当修改对应配置项。
  >
...
- 禁止在 in 或 instanceof 操作符的**左侧变量前**使用感叹号。
...
- setter **必须有**对应的 getter，getter **可以没有**对应的 setter。
...
- 数组的方法**除了 forEach 之外**，回调函数必须有返回值。
...
- 禁止在**正则表达式中出现形似除法操作符的开头**，如 let a = /=foo/
...
- 允许使用 **magic numbers**
  ...
  > [魔术数字](https://zh.wikipedia.org/zh-cn/魔術數字)是在代码中多次出现的没有明确含义的数字。它最好由命名常量取代。
...
- **允许**所有的 `var` 声明出现**不在**在它们所在的作用域顶部
...
- 在function，if，try等方法后使用**统一的大括号风格**
...
- 要求**逗号**放在数组元素、对象属性或变量声明**之后**，且在同一行
...
- 如果函数的**任一参数有换行**，则要求在函数括号内换行。否则禁止换行。
...
- 除关键字和保留字外，**满足其他规则（如变量驼峰命名规范）的情况下**，不限制标识符的使用。
...
- **满足其他规则（如变量驼峰命名规范）的情况下**，不限制标识符的使用。
...
- 通常**不允许**使用 Array的构造函数来创建数组。唯一的**例外**是通过给构造函数传入指定的一个数值来创建稀疏数组
...
- **派生类**中的构造函数**必须**调用 super()。**非派生类**的构造函数不能调用 super()
...
- 强制在 * 和 function 关键字之间**有空格**，* 之后无空格
...
- 禁止在**构造函数中**，在调用 super() 之前使用 this 或 super
...
- 禁止**在 import 和 export 和解构赋值时将引用重命名为相同的名字**
...
- 强制 .vue 文件里的template内**每行**最多**一个属性**
...
- 强制组件标签、void标签（img等）、svg标签、math标签**自我闭合**
